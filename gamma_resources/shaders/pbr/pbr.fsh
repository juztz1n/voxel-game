...
{ 
    ...
    vec2 P = vec2(viewDir.x, -viewDir.y) / -viewDir.z * pbr_Material.height_scale; 
    vec2 deltaTexCoords = P / numLayers;
  
    vec2  currentTexCoords     = texCoords;
    float currentDepthMapValue = texture(pbr_Material.height, currentTexCoords).r;
      
    while(currentLayerDepth < currentDepthMapValue)
    {
        currentTexCoords -= deltaTexCoords;
        currentDepthMapValue = texture(pbr_Material.height, currentTexCoords).r;  
    ...
    float beforeDepth = texture(pbr_Material.height, prevTexCoords).r - currentLayerDepth + layerDepth;
...
void main()
{	
    vec3 V = normalize(TBN * ViewPos - TBN * WorldPos);
    ...
    for(int i = 0; i < 32; ++i) 
    {
        vec3 L = normalize(TBN * input_PointLights[i].position - TBN * WorldPos);
        vec3 H = normalize(V + L);
        float distance = length(input_PointLights[i].position - TBN * WorldPos);
